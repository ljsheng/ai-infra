...
- **风险信号自动激活**：当检测到以下信号时，无需用户指令即可主动调用对应 skill：
  ...
  - 改动涉及 Shell 脚本 → `bash-defensive-patterns`（ShellCheck 检查已由 hook 自动执行）
...
| Shell 脚本编写 | `linux-shell-scripting`（功能实现） → `bash-defensive-patterns`（防御加固） → ShellCheck hook 自动检查 | 实现 → 加固 → 自动检查 |
...
| Shell 脚本 | `linux-shell-scripting`、`bash-defensive-patterns` | 生产级脚本编写、防御性编程（ShellCheck 由 hook 自动执行） |
