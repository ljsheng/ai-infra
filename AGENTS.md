...
- **上下文嗅探**：每轮对话开始时，根据工作区技术栈（语言、框架、工具链）和当前任务类型，主动收窄可用 skill 范围。检测规则如下：
  ...
  | `composer.json` 含 `phpstan` | `phpstan-analysis` |
  | `pom.xml` / `build.gradle` 含 `spring-boot` | `java-expert`、`java-junit` |
  | `requirements.txt` / `pyproject.toml` 存在 | `python-code-style`、`python-testing-patterns`、`python-type-safety` |
  | `go.mod` 存在 | `go-concurrency-patterns` |
  | `Cargo.toml` 存在 | `rust-async-patterns`、`memory-safety-patterns` |
  | `*.swift` / `Package.swift` 存在 | `swift-concurrency-expert`、`swiftui-*` 系列 |
  ...
  | `.eslintrc*` / `eslint.config.*` 存在 | `linting-neostandard-eslint9` |
...
- **风险信号自动激活**：当检测到以下信号时，无需用户指令即可主动调用对应 skill：
  ...
  - 改动涉及 PHP 代码 → `php-pro` + `phpstan-analysis`（写完跑 PHPStan）
  - 改动涉及 PHPDoc → `php-doc`（写 PHPDoc 前先加载规范）
  - 改动涉及 Shell 脚本 → `bash-defensive-patterns` + `shellcheck-configuration`
...
| Shell 脚本编写 | `linux-shell-scripting`（功能实现） → `bash-defensive-patterns`（防御加固） → `shellcheck-configuration`（静态检查） | 实现 → 加固 → 检查 |
...
| Shell 脚本 | `linux-shell-scripting`、`bash-defensive-patterns`、`shellcheck-configuration` | 生产级脚本编写、防御性编程、静态分析 |
